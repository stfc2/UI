...
if (isset($_POST['submitdata']))
{
    ...
    {
        // Retrive alliance ID
        $sql = 'SELECT alliance_id FROM user WHERE user_id = '.$_POST['id'];
        $user=$db->queryrow($sql);

        // If set, change alliance owner
        if (!isset($user['alliance_id']))
        {
            $sql = 'UPDATE user, alliance
                    SET user_alliance_status = 3, alliance_owner = '.$_POST['id'].',
                    ...
                    WHERE user_id = '.$_POST['id'].' AND alliance_id = '.$user['alliance_id'];
