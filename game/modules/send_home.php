...
if($starter_atkptc && $free_planet) {
    message(NOTICE, 'Mit aktiviertem Angriffsschutz dürfen keine unbewohnten Planeten angeflogen werden');
}
...
//$step = (!empty($_POST['step'])) ? $_POST['step'] : 'basic_setup';
        ...
        if($des_istardate < $min_istardate) {
            message(NOTICE, 'Die frühstmögliche Ankunft ist Sternzeit '.($game->config['stardate'] + ($min_time / 10)));
        }
